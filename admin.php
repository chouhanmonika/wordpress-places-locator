...
Description:  Search Post types by address within a certain radius.
...
//////////////////////////////
include_once 'wp-places-locator.php';

global $address_fields;
$address_fields = array();
$prefix = '_wppl_';
$options = get_option('wppl_fields');
$address_fileds = explode(",", $options['address_fields']);

$meta_box = array(
    'id' => 'wppl-meta-box',
    'title' => 'Address',
    'pages' => $address_fileds,
    'context' => 'normal',
    'priority' => 'high',
    'fields' => array(
        array(
...
function wppl_add_box() {
    global $meta_box;
 	...
 	foreach ($meta_box['pages'] as $page) {
        add_meta_box($meta_box['id'], $meta_box['title'], 'wppl_show_box', $page, $meta_box['context'], $meta_box['priority']);
    }
...
function wppl_save_data($post_id) {
    ...
    global $post;
    $options = get_option('wppl_fields'); 
	$address_fields = explode(",", $options['address_fields']);
    ...
    // Check permissions //
  	if (in_array($_POST['post_type'], $address_fields)) {
    ...
    $address_arg = $_POST[$meta_box['fields'][0]['id']] . " " . $_POST[$meta_box['fields'][1]['id']] . " " . $_POST[$meta_box['fields'][2]['id']] . " " . $_POST[$meta_box['fields'][3]['id']] . " " . $_POST[$meta_box['fields'][4]['id']] . " " . $_POST[$meta_box['fields'][5]['id']];    
...
function wppl_post_updated_messages_filter($messages) {
	global $post;
  		$messages['post'][99] = '<div id="error-message">Publish not allowed - the address you entered does not exist or some of the address fields are empty. please check the address fields again.</div>';
  		return $messages;
}
