...
function wppl_shortcode($params) {	
	...
	$check_posts = explode(",", $options['address_fields']);
   ...
   // $taxonomies = explode( ",", $taxonomies);
	...
	echo				'<p>';
	echo    				'<label for="wppl-post-type">' . $options['post_type_title'] . ': </label>';				
							/* post types dropdown */
	...
	echo	'<p>';
	echo        '<label for="wppl-address">' . $options['address_title'] . ': </label>';
	...
	echo    		'<p>';
	echo    			'<label for="wppl-distance">' . $options['distance_title'] . ': </label>';
...
function get_results_within( $org_address, $radius ) {
	...
	echo		'<div class="wppl-map-wrapper">';
					if($show_map == "1") {
						if($which_map == "plugin") {
	echo 					do_shortcode('[nearby]');
						} else {
							if ($which_map == "google") {
	echo						'<script src="http://maps.google.com/maps/api/js?sensor=false" type="text/javascript"></script>';
	echo						'<div id="map" style="width:' . $map_width .'px; height:'. $map_height . 'px;"></div>';
	echo 						'<script type="text/javascript">'; echo 'locations= '.json_encode($posts_within),';'; echo '</script>';
	echo 						'<script type="text/javascript">'; echo 'your_location= '.json_encode($your_loc),';'; echo '</script>';
	echo						'<script type="text/javascript">'; echo 'page= '.json_encode($from_page),';'; echo '</script>';
